...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-10">
          <h1 class="m-0 text-dark"><i class="fa fa-newspaper-o" aria-hidden="true"></i>&nbsp;Product Sub Detail</h1>
        </div>
        <div class="col-sm-2">
          <?php  
            $hide = DB::table('tb_product_sub_detail')->where('sub_pro_det_id','=',$id)->get();
            ...
            <a href="{{ url('insert_product_sub_detail/'.$id.'/'.$product_id.'/'.$delete_id.'') }}">
                <button {{ $disabled }} type="button" class="btn btn-dark"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;Insert
                </button>
            </a>
            <a href="{{ url('product_sub/'.$product_id.'/'.$delete_id.'') }}"><button type="button" class="btn btn-dark"><i class="fa fa-arrow-left" aria-hidden="true"></i>&nbsp;Back</button></a>
    ...
    <div class="work-con">
        <div class="tab-content" id="master-tabContent">
            <div id="con_1" aria-labelledby="tab_1" role="tabpanel" class="tab-pane fade show active">
                <table class="table table-bordered data-table" id="list_product_sub_detail" style="width: 100%;">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Title EN</th>
...
<script type="text/javascript">
  ...
  var token = $('meta[name="csrf-token"]').attr('content');
     ...
     var delete_id = document.getElementById("delete_id").value;
         $('#list_product_sub_detail').DataTable({
             scrollY: true,
             scrollCollapse: true,
             searching: true,
             "ajax": {
              "url": '{{url("product_sub_detail_data")}}',
              "type": 'GET',
              "data": {
                "id": id,
...
<script type="text/javascript">
  function ConfirmDelete(id,image){
    ...
    {
     $.ajax({
        url:"{{ route('delete_product_sub_detail')}}",
        method:"POST",
        data:{
        "_token": "{{ csrf_token() }}",
        id:id,
